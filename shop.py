...
async def handle_feedback_message(message: Message, state: FSMContext):
    ...
    if "expecting_feedback" in data and data["expecting_feedback"]:
        ...
        await bot.send_message(admin_user_id, f"Новый заказ от {user_id}:\n{feedback_message}")
        await message.answer("Ожидайте ответа от оператора!")
